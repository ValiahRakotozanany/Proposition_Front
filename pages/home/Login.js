...
const appelWebService = async () => {
  try {
    console.log(email);
    const response = await fetch(`http://26.22.221.140:8001/proposition/Login_Famille?email=${email}&motdepasse=${password}`);
...
const submit = () =>{
  ...
  console.log(email)
     fetch("http://26.22.221.140:8001/proposition/Login_Famille?email="+email+"&motdepasse="+password,//"http://localhost:8001/proposition/Login_Famille?email="+email+"&motdepasse="+password,
     {
         method:"POST",
